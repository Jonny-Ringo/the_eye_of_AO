...
function insertHistoricalBlock(blockHeight, timestamp, isWeekly)
    ...
    if isWeekly then
        dbAdmin:apply([[
            INSERT INTO WeeklyBlocks (date, timestamp, blockHeight)
            VALUES (?, ?, ?);
        ]], {dateStr, timestamp, blockHeight})
        print(string.format("Inserted weekly block %d for date %s", blockHeight, dateStr))
    else
        dbAdmin:apply([[
            INSERT INTO DailyBlocks (date, timestamp, blockHeight)
            VALUES (?, ?, ?);
        ]], {dateStr, timestamp, blockHeight})
...
function deleteBlockByDate(date)
    local result = dbAdmin:apply([[
        DELETE FROM DailyBlocks
        WHERE date = ?;
    ]], {date})

    print(string.format("Deleted block entry for date %s", date))
