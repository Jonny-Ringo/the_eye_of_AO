...
/**
 * Generates GraphQL queries for Arweave transaction analytics
 * @param {string} source - Source type: 'total', 'redstone', 'ardrive', 'ao', 'ario'
 ...
 */
export function generateArweaveTransactionQuery(source, startHeight, endHeight, currentHeight) {
    const blockRange = endHeight === currentHeight
        ? `block: { min: ${startHeight} }`
        : `block: { min: ${startHeight}, max: ${endHeight} }`;
    ...
    switch(source) {
        case 'redstone':
            tagFilter = 'tags: [{ name: "Bundler-App-Name", values: ["Redstone"] }]';
            break;
        case 'ardrive':
            tagFilter = 'tags: [{ name: "Bundler-App-Name", values: ["ArDrive"] }]';
            break;
        case 'ao':
            tagFilter = 'tags: [{ name: "Data-Protocol", values: ["ao"] }]';
            break;
        case 'ario':
            tagFilter = 'tags: [{ name: "Bundler-App-Name", values: ["AR.IO Network"] }]';
            break;
        case 'total':
        default:
            tagFilter = '';
    ...
    const query = tagFilter
        ? `query { transactions(${tagFilter}, ${blockRange}) { count } }`
        : `query { transactions(${blockRange}) { count } }`;
...
export function getProcessDisplayName(processName) {
    // Some charts are not standard PROCESSES-driven charts
    if (processName === 'arweaveTransactions') return 'Arweave Transaction Analytics (With AO Messages)';
