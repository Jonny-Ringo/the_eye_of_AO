...
export function setupTimeRangeButtons(fetchDataCallback, fetchWeeklyCallback) {
    ...
    processNames.forEach(processName => {
        // Set arweaveTransactions to default to 1M, all others use DEFAULT_TIME_RANGE
    ...
    chartCards.forEach(card => {
        ...
        const defaultRange = processName === 'arweaveTransactions' ? '1M' : DEFAULT_TIME_RANGE;
        const defaultButton = Array.from(buttons).find(btn =>
            btn.textContent.trim() === defaultRange
        );
        if (defaultButton) {
